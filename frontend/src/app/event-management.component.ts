...
export class EventManagementComponent implements OnInit {
  ...
  getFilteredRows(): Record<string, any>[] {
    ...
    if (term) {
      ...
      filteredData = filteredData.filter(row => colsToSearch.some(col =>
  ...
  private shortenColumnName(column: string): string {
    const words = column.trim().split(/\s+/).filter(Boolean);
    if (words.length <= 3 && column.length <= 20) {
      return column;
    }
    const threeWords = words.slice(0, 3).join(' ');
    return threeWords.length <= 20 ? threeWords : threeWords.slice(0, 20).trim();
  }
